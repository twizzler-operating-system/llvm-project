//===--- Twizzler.cpp - Twizzler ToolChain Implementations --------*- C++ -*-===//
//
...
void twizzler::Linker::ConstructJob(Compilation &C, const JobAction &JA,
                                   const InputInfo &Output,
                                   const InputInfoList &Inputs,
                                   const ArgList &Args,
                                   const char *LinkingOutput) const {
  const toolchains::Twizzler &ToolChain =
      static_cast<const toolchains::Twizzler &>(getToolChain());
...
/// Twizzler - Twizzler tool chain which can call as(1) and ld(1) directly.

Twizzler::Twizzler(const Driver &D, const llvm::Triple &Triple,
                 const ArgList &Args)
...
Tool *Twizzler::buildLinker() const {
  return new tools::twizzler::Linker(*this);
}

ToolChain::RuntimeLibType Twizzler::GetRuntimeLibType(
...
Twizzler::GetCXXStdlibType(const ArgList &Args) const {
...
void Twizzler::addClangTargetOptions(const ArgList &DriverArgs,
...
void Twizzler::AddClangSystemIncludeArgs(const ArgList &DriverArgs,
...
void Twizzler::AddCXXStdlibLibArgs(const ArgList &Args,
